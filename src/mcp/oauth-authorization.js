...
function errorResponse(error, description, status = 400) {
  return Response.json({ error, ...(description ? { error_description: description } : {}) }, {
    status,
...
function connectorForResource(resource) {
  return new URL(resource).pathname.match(/^\/mcp\/([a-zA-Z0-9_-]+)$/)?.[1] || null;
}
...
export async function handleMcpAuthorize(request, env) {
  ...
  if (!clientId || !redirectUri || !codeChallenge || codeChallengeMethod !== "S256") {
    return errorResponse("invalid_request", "client_id, redirect_uri, code_challenge and S256 code_challenge_method are required");
  }
