...
export async function handleMcpTokenSync(request, env) {
  ...
  if (request.method === "DELETE") {
    await deleteTokens(env.TOKENS_KV, provider, userId);
    return Response.json({ ok: true, provider, revoked: true }, { headers: headers() });
  }

  if (request.method !== "POST") {
    return Response.json({ error: "Method not allowed" }, { status: 405, headers: { ...headers(), allow: "POST, DELETE" } });
  }
  ...
  await saveTokens(env.TOKENS_KV, provider, tokens, userId, encryptionSecret);

  const expiresAt = tokens.expires_at || (tokens.expires_in ? Date.now() + tokens.expires_in * 1000 : null);
  return Response.json(
    { ok: true, provider, userId, stored: true, expires_at: expiresAt },
    { headers: headers() }
  );
}
