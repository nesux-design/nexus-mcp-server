...
const STATE_TTL_SECONDS = 600; const encoder = new TextEncoder(); const decoder = new TextDecoder();
...
export async function handleOAuth(request, env, path) {
  ...
  const url = new URL(request.url); const isCallback = match[2] === "callback" || (!match[2] && (url.searchParams.has("code") || url.searchParams.has("error")));
  if (!isCallback) {
    const userId = await requireInternalUser(request, env); if (!userId) return Response.json({ error: "Unauthorized" }, { status: 401, headers: securityHeaders() });
    const mcpAuth = mcpAuthFromUrl(url); const encryptionSecret = env.NEXUS_TOKEN_ENCRYPTION_SECRET || env.NEXUS_INTERNAL_AUTH_SECRET;
    if (mcpAuth) { const existing = await loadTokens(env, provider, userId, encryptionSecret); if (existing?.access_token) return await finishMcpAuthorization(request, env, provider, mcpAuth, userId); }
    if (!env.OAUTH_CODES) return Response.json({ error: "OAuth durable storage is not configured" }, { status: 503, headers: securityHeaders() });
    if (connector.pkce) { const verifier = createPkceVerifier(); const challenge = await createPkceChallenge(verifier); const state = await createState(env, provider, userId, { verifier, mcpAuth }); return Response.redirect(authorizationUrl(request, env, provider, state, challenge).toString(), 302); }
    const state = await createState(env, provider, userId, { mcpAuth }); return Response.redirect(authorizationUrl(request, env, provider, state).toString(), 302);
  }
  const error = url.searchParams.get("error"); if (error) return Response.json({ error, description: url.searchParams.get("error_description") }, { status: 400, headers: securityHeaders() });
  const code = url.searchParams.get("code"); const state = url.searchParams.get("state"); if (!code || !state) return new Response("Missing OAuth code or state", { status: 400, headers: securityHeaders() });
  const stateRecord = await consumeState(env, state, provider); if (!stateRecord) return new Response("Invalid or expired OAuth state", { status: 400, headers: securityHeaders() });
  try {
    const tokens = await exchangeCode(request, env, provider, code, stateRecord.verifier); if (!tokens?.access_token) throw new Error("OAuth token response did not include access_token");
    const encryptionSecret = env.NEXUS_TOKEN_ENCRYPTION_SECRET || env.NEXUS_INTERNAL_AUTH_SECRET; await saveTokens(env, provider, tokens, stateRecord.userId, encryptionSecret);
  ...
  } catch (err) { console.error("TOKEN EXCHANGE ERROR:", err.message); return Response.json({ error: "OAuth token exchange failed", debugMessage: err.message }, { status: 502, headers: securityHeaders() }); }
}
