...
async function handleLocalMcpTools(ServerClass, env, userId, requestId) { const server = new ServerClass(env); const tools = server.getToolDefinitions(); return Response.json({ tools }, { status: 200, headers: jsonHeaders(requestId) }); }
...
export default { async fetch(request, env) { const requestId = crypto.randomUUID(); const url = new URL(request.url); const pathname = url.pathname; try {
  ...
  const oauthResponse = await handleOAuth(request, env, pathname); if (oauthResponse) { const headers = new Headers(oauthResponse.headers); headers.set("x-request-id", requestId); return new Response(oauthResponse.body, { status: oauthResponse.status, statusText: oauthResponse.statusText, headers }); }
