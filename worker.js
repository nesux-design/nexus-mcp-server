...
async function handleLocalMcpCall(ServerClass, request, env, userId, requestId) {
  ...
  if (!toolName) return Response.json({ error: "tool parameter is required" }, { status: 400, headers: jsonHeaders(requestId) });
  const server = new ServerClass(env);
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (metadataMatch && request.method === "GET") {
        if (!publicConnectorList().some((item) => item.id === metadataMatch[1])) return new Response("Not Found", { status: 404, headers: baseHeaders(requestId) });
      ...
      if (oauthResponse) {
        ...
        return new Response(oauthResponse.body, { status: oauthResponse.status, statusText: oauthResponse.statusText, headers });
      }
      ...
      if (localToolsMatch && request.method === "POST" && LOCAL_MCP_SERVERS[localToolsMatch[1]]) {
        ...
        if (!userId) return Response.json({ error: "Unauthorized" }, { status: 401, headers: jsonHeaders(requestId) });
      ...
      return Response.json({ error: "Internal server error", requestId }, { status: 500, headers: jsonHeaders(requestId) });
    }
  }
};
