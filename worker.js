...
export default { async fetch(request, env) { const requestId = crypto.randomUUID(); const url = new URL(request.url); const pathname = url.pathname; try {
  ...
  if (["/.well-known/oauth-authorization-server", "/.well-known/oauth-authorization-server/oauth", "/.well-known/openid-configuration/oauth", "/oauth/.well-known/openid-configuration"].includes(pathname) && request.method === "GET") { const response = oauthAuthorizationServerMetadata(request); response.headers.set("x-request-id", requestId); response.headers.set("x-nexus-version", VERSION); return response; }
  if (pathname === "/oauth/authorize") { try { const response = await handleMcpAuthorize(request, env); const headers = new Headers(response.headers); headers.set("x-request-id", requestId); headers.set("x-nexus-version", VERSION); return new Response(response.body, { status: response.status, statusText: response.statusText, headers }); } catch (error) { console.error("MCP OAuth authorization route failed", { error: error instanceof Error ? error.message : String(error) }); return oauthServerError(requestId, "OAuth authorization is temporarily unavailable"); } }
  if (pathname === "/oauth/token") { try { const response = await handleMcpToken(request, env); const headers = new Headers(response.headers); headers.set("x-request-id", requestId); headers.set("x-nexus-version", VERSION); return new Response(response.body, { status: response.status, statusText: response.statusText, headers }); } catch (error) { console.error("MCP OAuth token route failed", { error: error instanceof Error ? error.message : String(error) }); return oauthServerError(requestId, "OAuth token service is temporarily unavailable"); } }
  ...
  const realMcpMatch = pathname.match(/^\/mcp\/([a-zA-Z0-9_-]+)$/); if (realMcpMatch) { const response = await handleRealMcp(request, env, realMcpMatch[1]); response.headers.set("x-request-id", requestId); return response; }
  const localToolsMatch = pathname.match(/^\/([a-zA-Z0-9_-]+)\/tools$/); if (localToolsMatch && request.method === "POST" && LOCAL_MCP_SERVERS[localToolsMatch[1]]) { const auth = await resolveLegacyUser(request, env, localToolsMatch[1]); if (auth.response) return auth.response; return await handleLocalMcpTools(LOCAL_MCP_SERVERS[localToolsMatch[1]], env, auth.userId, requestId); }
  const localCallMatch = pathname.match(/^\/([a-zA-Z0-9_-]+)\/call$/); if (localCallMatch && request.method === "POST" && LOCAL_MCP_SERVERS[localCallMatch[1]]) { const auth = await resolveLegacyUser(request, env, localCallMatch[1]); if (auth.response) return auth.response; return await handleLocalMcpCall(LOCAL_MCP_SERVERS[localCallMatch[1]], request, env, auth.userId, requestId); }
